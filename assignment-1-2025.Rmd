...
-->
This plot does not show any clear trend between bill depth and bill length.
...
-->
With colour added to the plot we can see clusters that correspond to each species, showing that each species has a particular bill depth and length
...
```{r}
p = ggplot(data = penguins, mapping = aes(x=bill_length_mm, y=bill_depth_mm, color=species)) +
  geom_point()
p + labs(title="Bill length and Depth for Species of Penguin",
         x="Bill Length (mm)",
         y="Bill Depth (mm)",
         )
```
...
```{r}
q = ggplot(data = penguins, mapping = aes(x=flipper_length_mm, y=body_mass_g, color=species)) +
  geom_point()
q + labs(title="Flipper length and Body Mass for Species of Penguins",
         x="Flipper Length (mm)",
         y="Body Mass (g)",
         )
```
